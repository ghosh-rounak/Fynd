...
class SearchViewModel(private val repository: SearchRepository) : ViewModel(), Observable {
    ...
    private suspend fun safeMoviesCall(pageNumber: Int) {
        ...
        if (moviesPage <= lastPage) {
            ...
            try {
                val response = repository.getSearchedMovies(query,pageNumber)
    ...
    private fun handleMoviesResponse(response: Response<SearchedMoviesResponse>) {
    ...
    private fun handleRefreshMoviesResponse(response: Response<SearchedMoviesResponse>) {
        if (response.isSuccessful) {
            response.body()?.let { resultResponse ->
                lastPage = resultResponse.totalPages
                moviesPage++
    ...
    fun onRefreshClick() {
        Log.d("onCreate", "onRefreshClick : From SearchViewModel ")
        getMovies()
    }
